...

    {
        ...
        try {
            ...
            if ($this->getBatchParams('create_shipment') == "true") {
                ...
                $this->createShipment($order1, $itemQty);
            ...
            foreach($order1->getQuote()->getAllItems() as $quoteItem) {
                ...
                if($quoteItem->getProduct()->getSku() == $key) {
...
//                $option->save();
        ...
        if ($customer->getId()) {
			...
			if ($this->getBatchParams('update_customer_address') == "true") {
				...
				$dataBilling = array(
					...
					'street_full'  => $importData['billing_street_full'],
					'street_2'  => $importData['billing_street_2'],
                    ...
                    foreach($options as $option) {
						...
						if($option->getType() == "drop_down" || $option->getType() == "radio" || $option->getType() == "checkbox" || $option->getType() == "multiple") {
							...
							foreach ($values as $value) {
									#echo "TITLECSV: " . $parts[$i + 2] . " - " . $value->getTitle() . "<br/>";
								if ($value->getTitle() == $parts[$i + 2]) {
								...
								#echo "TITLE2: " . $parts[$i + 2];
								$simple_custom_option_values[$option->getId()] = $parts[$i + 2];
								$itemtrue = true;
    ...
    public function createShipment($order1, $itemQty)
