        ...
        if ($data) {
            ...
            try {
                ...
                if (!isset($data['product']['options']) || $this->_isEmptyOptions($data['product']['options'])) {
                    ...
                    $data['general']['hash_options'] = serialize($this->_prepareOptions($productOptions));
                }
                if ($error) {
                ...
                if ($productOptions && isset($productIds) && is_array($productIds)) {
                    ...
                    if (count($productIds) > 0) {
                        $optionIds = array();
                        $optionIds = $productOption->saveProductOptions($productOptions, $productIds, $group, false);
                    } else {
                        $relation = Mage::getResourceSingleton('customoptions/relation');
                        $productIdsToDelete = $relation->getProductIds($group->getId());
                        $productOption->deleteProductsFromOptions($productOptions, $productIdsToDelete, $group);
                    }
                }
