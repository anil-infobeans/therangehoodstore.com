...
// BOF Separate Pricing Per Customer, tax exempt modification
    ...
    global $customer_zone_id, $customer_country_id, $sppc_customer_group_tax_exempt, $sppc_customer_group_tax_rates_exempt;
     ...
     if ($customer_group_tax_exempt == '1') {
	     foreach ($sppc_customer_group_tax_rates_exempt as $key => $value) {
		 	$tax_exempt_values .= "and tr.tax_rates_id != '" . $key . "' "; 
		 }
     }
     /* TAX RATES EXEMPTABLE EOC */	 
// EOF Separate Pricing Per Customer, tax exempt modification
    ...
    $tax_query = tep_db_query("select sum(tax_rate) as tax_rate from " . TABLE_TAX_RATES . " tr left join " . TABLE_ZONES_TO_GEO_ZONES . " za on (tr.tax_zone_id = za.geo_zone_id) left join " . TABLE_GEO_ZONES . " tz on (tz.geo_zone_id = tr.tax_zone_id) where (za.zone_country_id is null or za.zone_country_id = '0' or za.zone_country_id = '" . (int)$country_id . "') and (za.zone_id is null or za.zone_id = '0' or za.zone_id = '" . (int)$zone_id . "') and tr.tax_class_id = '" . (int)$class_id . "' ".$tax_exempt_values."group by tr.tax_priority");	
     /* TAX RATES EXEMPTABLE EOC */	
...
// TABLES: address_format
  function tep_address_format($address_format_id, $address, $html, $boln, $eoln) {
    ...
    $zip = $postcode;
					...
					}elseif(strlen($address['email_address'])>0){
						$email=tep_output_string_protected($address['email_address']);
					}
...
// TABLES: customers, address_book
  function tep_address_label($customers_id, $address_id = 1, $html = false, $boln = '', $eoln = "\n") {
    $address_query = tep_db_query("select entry_firstname as firstname, entry_lastname as lastname, entry_company as company, entry_street_address as street_address, entry_suburb as suburb, entry_city as city, entry_postcode as postcode, entry_state as state, entry_zone_id as zone_id, entry_country_id as country_id, entry_email_address as email from " . TABLE_ADDRESS_BOOK . " where customers_id = '" . (int)$customers_id . "' and address_book_id = '" . (int)$address_id . "'");
...
// OTF contrib ends
				...
				$gc_code=substr(ereg_replace('[\$\/\.]', '', crypt(strval(time()))), 0, 11);
				return $gc_code;
				}
