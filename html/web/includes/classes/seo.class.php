 ...
 */        
        function SEO_URL_INSTALLER(){
                ...
                $this->default_config['SEO_ADD_CAT_PARENT'] = array('DEFAULT' => 'false',
                                      'QUERY' => "INSERT INTO `".TABLE_CONFIGURATION."` VALUES (NULL, 'Add category parent to begining of URLs?', 'SEO_ADD_CAT_PARENT', 'false', 'This setting will add the category parent(s) name to the beginning of the category URLs (i.e. - parent-category-w-1.html).', GROUP_INSERT_ID, ".$x.", NOW(), NOW(), NULL, 'tep_cfg_select_option(array(''true'', ''false''),')");
                $x++;
                ...
                $this->default_config['USE_SEO_CACHE_PAGE_EDITOR'] = array('DEFAULT' => 'true',
                                      'QUERY' => "INSERT INTO `".TABLE_CONFIGURATION."` VALUES (NULL, 'Enable Page Editor cache?', 'USE_SEO_CACHE_PAGE_EDITOR', 'true', 'This will turn off caching for the Page Editor pages.', GROUP_INSERT_ID, ".$x.", NOW(), NOW(), NULL, 'tep_cfg_select_option(array(''true'', ''false''),')");
                $x++;
                $this->default_config['USE_SEO_CACHE_NEWS'] = array('DEFAULT' => 'true',
                                      'QUERY' => "INSERT INTO `".TABLE_CONFIGURATION."` VALUES (NULL, 'Enable Page Editor cache?', 'USE_SEO_CACHE_NEWS', 'true', 'This will turn off caching for the News pages.', GROUP_INSERT_ID, ".$x.", NOW(), NOW(), NULL, 'tep_cfg_select_option(array(''true'', ''false''),')");
                $x++;
...
//ojp FILENAME_LINKS
                $seo_pages = array(FILENAME_DEFAULT,
								   ...
								   if ( defined('FILENAME_NEWS') ) $seo_pages[] = FILENAME_NEWS;
...
//ojp USE_SEO_CACHE_LINKS
                $this->attributes = array('PHP_VERSION' => PHP_VERSION,
										  ...
										  'USE_SEO_CACHE_NEWS' => defined('USE_SEO_CACHE_NEWS') ? USE_SEO_CACHE_NEWS : 'false',
...
//ojp generate_link_cache
                ...
                if ($this->attributes['USE_SEO_CACHE_GLOBAL'] == 'true'){
                        ...
                        if ( $this->attributes['USE_SEO_CACHE_PAGE_EDITOR'] == 'true' && defined('TABLE_NEWS')) $this->generate_news_cache();
                ...
                foreach ($p as $index => $valuepair){
                        ...
                        switch ($p2[0]){ 
                                ...
                                case 'info_id': //Information Pages
                       			...
                       			case 'id_news': // Page Editor
								
                       					switch(true){
                       						case ($page == FILENAME_NEWS):
                       							$url = $this->make_url($page, $this->get_news_name($p2[1]), $p2[0], $p2[1], '.html');
                       							break;
                       						default: 
                       							$container[$p2[0]] = $p2[1];
                       							break;
              ...
              $cName = $this->get_all_category_parents($pID, $cName);
                ...
                switch(true){
                        ...
                        default:
                                ...
                                $sqlCmd = $this->attributes['USE_SEO_HEADER_TAGS'] == 'true' ? 'products_t3 as pName' : 'products_t3 as pName';
                                $sql = "SELECT " . $sqlCmd . "
                        ...
                        default:
                                ...
                                switch(true){
                                        case ($this->attributes['SEO_ADD_CAT_PARENT'] == 'true'):                                        
                                           $sqlCmd = $this->attributes['USE_SEO_HEADER_TAGS'] == 'true' ? 'LOWER(cd.categories_htc_title_tag) as cName' : 'LOWER(cd.categories_name) as cName';
 ...
 */	
	...
	function get_news_name($newsID){
		switch(true){
			case ($this->attributes['USE_SEO_CACHE_GLOBAL'] == 'true' && defined('NEWS_' . $newsID)):
				if ($this->attributes['USE_SEO_PERFORMANCE_CHECK'] == 'true') $this->performance['CACHE_QUERY_SAVINGS']++;
				$return = constant('NEWS_' . $newsID);
				$this->cache['NEWS'][$newsID] = $return;
				break;
			case ($this->attributes['USE_SEO_CACHE_GLOBAL'] == 'true' && isset($this->cache['NEWS'][$newsID])):
				if ($this->attributes['USE_SEO_PERFORMANCE_CHECK'] == 'true') $this->performance['CACHE_QUERY_SAVINGS']++;
				$return = $this->cache['NEWS'][$newsID];
				break;
			default:
				if ($this->attributes['USE_SEO_PERFORMANCE_CHECK'] == 'true') $this->performance['NUMBER_QUERIES']++;
				$sql = "SELECT h1_news as pmName 
						FROM  news_description 
						WHERE id_news ='".(int)$newsID."' 
						AND id_language ='".(int)$this->languages_id."' 
						LIMIT 1";
				$result = $this->DB->FetchArray( $this->DB->Query( $sql ) );
				$newsName = $this->strip( $result['pmName'] );
				$this->cache['NEWS'][$newsID] = $newsName;
				if ($this->attributes['USE_SEO_PERFORMANCE_CHECK'] == 'true') $this->performance['QUERIES']['NEWS'][] = $sql;
				$return = $newsName;
				break;								
		} # end switch		
		return $return;
 ...
 */        
        function generate_products_cache(){
                ...
                $sqlCmd = $this->attributes['USE_SEO_HEADER_TAGS'] == 'true' ? 'pd.products_head_title_tag as name' : 'pd.products_t3 as name';
 ...
 */	
	function generate_page_editor_cache(){
		...
		if ( !$is_cached || $is_expired ) { // it's not cached so create it
			...
			while ($pages = $this->DB->FetchArray($pages_query)) {
				$define = 'define(\'NEWS_' . $pages['id'] . '\', \'' . $this->strip($pages['name']) . '\');';
				$pages_cache .= $define . "\n";
				eval("$define");
			}
	...
	function generate_news_cache(){
		$this->is_cached($this->cache_file . 'NEWS', $is_cached, $is_expired);  	
		if ( !$is_cached || $is_expired ) { // it's not cached so create it
			$sql = "SELECT id_news as id, title_news as name 
					FROM news_description  
					WHERE language_id='".(int)$this->languages_id."'";
			$news_query = $this->DB->Query( $sql );
			$news_cache = '';
			while ($news = $this->DB->FetchArray($news_query)) {
				$define = 'define(\'NEWS_' . $news['id'] . '\', \'' . $this->strip($news['name']) . '\');';
				$news_cache .= $define . "\n";
				eval("$define");
			}
			$this->DB->Free($news_query);
			$this->save_cache($this->cache_file . 'NEWS', $news_cache, 'EVAL', 1 , 1);
			unset($news_cache);
		} else {
			$this->get_cache($this->cache_file . 'NEWS');		
		}
	} # end function  
 ...
 */        
        function check_redirect(){
                ...
                /**** redirect child path to full path - i.e., -w-3782.html to -w-28_3782.html, when applicable ****/
                if (strpos($this->attributes['SEO_REDIRECT']['URI_PARSED']['path'], '.html') !== FALSE) {
                    $u1 = $this->attributes['SEO_REDIRECT']['URI_PARSED']['path'];
               
                    if (($pStart = strpos($u1, "-w-")) !== FALSE) {         //start isolating the ID - only for categories
                       if (($pStop = strpos($u1, ".html")) !== FALSE) {
                          $path = substr($u1, $pStart, $pStop);             //will be something like -w-34.html
                          if (($pStart = strpos($path, "-")) !== FALSE) {   //isolate to the number
                              if (($pStop = strpos($path, ".html")) !== FALSE) {
                                  ...
                                  if (strpos($this->attributes['SEO_REDIRECT']['URI_PARSED']['path'], '-w-'.$actualPath) !== FALSE) { //this is the actual url
                ...
                if (strpos($this->attributes['SEO_REDIRECT']['URI_PARSED']['path'], '.html') !== FALSE) {
                    ...
                    if (($pStart = strpos($u1, "-w-")) !== FALSE) {         //start isolating the ID - only for categories
                       if (($pStop = strpos($u1, ".html")) !== FALSE) {
                          $path = substr($u1, $pStart, $pStop + 5);             //will be something like -w-34.html
                        ...
                        case ($this->uri != $this->real_uri && !$this->not_null($this->path_info)):
                                if (($pStart = strpos($this->uri_parsed['path'], "-p-")) !== FALSE) {
                                    if (($pStop = strpos($this->uri_parsed['path'], ".html")) !== FALSE) {
                                       ...
                                       $sqlCmd = $this->attributes['USE_SEO_HEADER_TAGS'] == 'true' ? 'products_head_title_tag as pName' : 'products_t3 as pName';
                                       $sql = "SELECT " . $sqlCmd . "
                                             FROM ".TABLE_PRODUCTS_DESCRIPTION."
                                             WHERE products_id='".(int)$pID."'
                                             AND language_id='".(int)$this->languages_id."'
                                ...
                                else if (($pStart = strpos($this->uri_parsed['path'], "-w-")) !== FALSE) {
                                    if (($pStop = strpos($this->uri_parsed['path'], ".html")) !== FALSE) {
                                       ...
                                       if ($forceRedirect || ($cName !== substr($this->uri_parsed['path'], 0, $pStart))) {
                                         $this->uri_parsed['path'] = $cName . "-w-" . $cID . ".html";
                                         $this->need_redirect = true;
                                         $this->do_redirect();
                                       }
                                    }
