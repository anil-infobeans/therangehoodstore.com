...
*/
  ...
  class order {
    ...
    function query($order_id) {
      ...
      $order_query = tep_db_query("select customers_id, customers_name, customers_company, customers_street_address, customers_suburb, customers_city, customers_postcode, customers_state, customers_country, customers_telephone, customers_email_address, customers_address_format_id, delivery_name, delivery_company, delivery_street_address, delivery_suburb, delivery_city, delivery_postcode, delivery_state, delivery_country, delivery_address_format_id, billing_name, billing_company, billing_street_address, billing_suburb, billing_city, billing_postcode, billing_state, billing_country, billing_address_format_id, payment_method, cc_type, cc_owner, cc_number, cc_expires, currency, currency_value, date_purchased, orders_status, last_modified, gc, gc_code from " . TABLE_ORDERS . " where orders_id = '" . (int)$order_id . "'");
      ...
      $this->info = array('currency' => $order['currency'],
				...
				'gc' => $order['gc'],
				'gc_code' => $order['gc_code'],
				'gc_redeemed' => $order['gc_redeemed'],
...
// EOF Separate Pricing Per Customer
		...
		$this->info['giftvoucher_name']["{$this->products[$index]['tax']}"] = '1';
    ...
    function cart() {
      global $HTTP_POST_VARS, $customer_id, $sendto, $billto, $cart, $languages_id, $currency, $currencies, $shipping, $payment, $gc, $gc_code, $gc_redeemed;
      ...
      $shipping_address_query = tep_db_query("select ab.entry_firstname, ab.entry_lastname, ab.entry_company, ab.entry_street_address, ab.entry_suburb, ab.entry_postcode, ab.entry_city, ab.entry_zone_id, z.zone_name, ab.entry_country_id, c.countries_id, c.countries_name, c.countries_iso_code_2, c.countries_iso_code_3, c.address_format_id, ab.entry_state, ab.entry_email_address  from " . TABLE_ADDRESS_BOOK . " ab left join " . TABLE_ZONES . " z on (ab.entry_zone_id = z.zone_id) left join " . TABLE_COUNTRIES . " c on (ab.entry_country_id = c.countries_id) where ab.customers_id = '" . (int)$customer_id . "' and ab.address_book_id = '" . (int)$sendto . "'");
...
$this->info['tax'] += $products_tax*$shown_price/100;
...
//      if (DISPLAY_PRICE_WITH_TAX == 'true') {
 ...
 //agregar zone_geo para filtrar el r.e. por zonas de impuestos - begin
    $orders_customers_groups_tax_rates_geo_zone_query = tep_db_query("select geo_zone_id from " . TABLE_ZONES_TO_GEO_ZONES . "  where zone_id = '". $shipping_address['entry_zone_id'] ."' and zone_country_id = '" . $shipping_address['entry_country_id'] . "'");
	$orders_customers_groups_tax_rates_geo_zone = tep_db_fetch_array($orders_customers_groups_tax_rates_geo_zone_query);
	$geo_zone_capture = $orders_customers_groups_tax_rates_geo_zone['geo_zone_id'];
 //agregar zone_geo para filtrar el r.e. por zonas de impuestos - end
...
global $sppc_customer_group_id;
  ...
  $customer_group_id = '0';
   ...
   if ($customer_group_id != '0'){
   $orders_customers_groups_tax_rates_query = tep_db_query("select tr.tax_rate, tr.tax_description, tr.tax_zone_id from " . TABLE_TAX_RATES . " tr inner join " . TABLE_CUSTOMERS_GROUPS_TAX_RATES_EXEMPT . " te on tr.tax_rates_id = te.tax_rates_id where te.customers_group_id = '". $customer_group_id ."' and tr.tax_zone_id = '". $geo_zone_capture . "'");

   if ($orders_customers_groups_tax_rates = tep_db_fetch_array($orders_customers_groups_tax_rates_query)) {
   $taxrea = $orders_customers_groups_tax_rates['tax_rate']/100;
   $taxreadescription = $orders_customers_groups_tax_rates['tax_description'];
   }
   

$gift_sql_query = tep_db_query("select coupons.*,coupon_email_track.*  from coupons,coupon_email_track  where coupons.coupon_id = coupon_email_track.coupon_id and coupons.coupon_code='".$this->info['comments']."' and coupon_email_track.emailed_to ='".$this->customer['email_address']."'");

	if ($gift_sql = tep_db_fetch_array($gift_sql_query)) {
	
	$check_gift_sql_query = tep_db_query("select coupon_id  from coupon_redeem_track  where coupon_id ='".$gift_sql['coupon_id']."'");
	
	if (tep_db_num_rows($check_gift_sql_query) == 0) {
			global $giftprice_id;
			$giftprice_id =  $gift_sql['coupon_id'];
			$giftprice = $gift_sql['coupon_amount'];
			$giftdescription = "Tarjeta de Regalo (-)";
			$mostrargift = 1;
		} else {
			global $giftprice_id;
			$giftprice_id =  0;
			$giftprice = 0.001;
			$giftdescription = "Tarjeta de Regalo ( invalido )";
			$mostrargift = 1;	
		}
	...
	if($this->info['comments']==""){
			...
			if($mostrargift == 1){
				$this->info['tax_groups']["$giftdescription"] += $giftprice ;//muestra el impuesto R.E.
			}

				
				if ((DISPLAY_PRICE_WITH_TAX == 'true') && ($customer_group_show_tax == '1')) {

					if($this->info['subtotal'] + $this->info['shipping_cost'] < $giftprice){				
						$this->info['giftvoucher'] += $this->info['subtotal'] + $this->info['shipping_cost'] ;											
					} else {
						$this->info['giftvoucher'] += $giftprice ;//CALCULA EL IMPUESTO R.E. PARA MAS ADELANE EN INFOTOTAL SUMARLO
					}
					
					$this->info['total'] = $this->info['subtotal'] + $this->info['shipping_cost'] - $this->info['giftvoucher'];				
					
				} else {
				
				  	$this->info['tax_groups']["$taxreadescription"] += $taxrea * $this->info['subtotal'];        
					$this->info['tax'] += $taxrea * $this->info['subtotal']; 
				
					if($this->info['subtotal'] + $this->info['tax'] + $this->info['shipping_cost'] < $giftprice){				
						$this->info['giftvoucher'] += $this->info['subtotal'] + $this->info['tax'] + $this->info['shipping_cost'] ;											
					} else {
						$this->info['giftvoucher'] += $giftprice ;//CALCULA EL IMPUESTO R.E. PARA MAS ADELANE EN INFOTOTAL SUMARLO
					}
								
					$this->info['total'] = $this->info['subtotal'] + $this->info['tax'] + $this->info['shipping_cost'] - $this->info['giftvoucher'];

				}
		

       if ($this->info['total'] == 0){
	      $this->info['payment_method']="Gift Certificate";
       }	  
