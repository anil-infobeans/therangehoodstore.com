...
*/
 ...
 class osC_CategoryTree_menu {
   var $root_category_id = 0,
       $max_level = 0,
       $data = array(),
       $root_start_string = '',
       $root_end_string = '',
       $parent_start_string = '<ul>',
   ...
   function osC_CategoryTree_menu($load_from_database = true) {
     global $languages_id;
   ...
   function buildBranch($parent_id, $level = 0, $cuentalevel = 0, $cuentalevelul = 0) {
   
	   if ($level == 0) {
		 $result = $this->parent_group_start_string;
		 $cuentalevel = $this->cuenta_level;
		}
     ...
     if (isset($this->data[$parent_id])) {
       foreach ($this->data[$parent_id] as $category_id => $category) {
		 ...
		 if ($level != 0) {
		  	if ($cuentalevel == $level) {			  
			  $cuentalevelul = $cuentalevelul+1;
				  if($cuentalevelul==1){
         ...
         if (isset($this->data[$category_id]) && (($this->max_level == '0') || ($this->max_level > $level+1))) {
		   $cuentalevel = $level+1;
           $result .= $this->buildBranch($category_id, $level+1, $cuentalevel, $cuentalevelul);
         } 
		 
		 $result .= $this->child_end_string;
		  if ($level != 0) {
		  	if ($cuentalevel == $level) {
				if($cuentalevelul==count($this->data[$parent_id])){
