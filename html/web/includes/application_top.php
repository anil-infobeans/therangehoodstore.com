...
//  funcion adicional html texto editor
...
// customization for the design layout
  define('BOX_TOTAL', 1024);
  define('BOX_WIDTH_1', 244);
  define('BOX_WIDTH_2', 780);// how wide the boxes should be in pixels (default: 125)
...
//$lng->get_browser_language(); ESTE EL ANTERIOR
// inicio fixeo
...
// End get 1 free
      ...
      case 'buy_now' :        if (isset($HTTP_GET_VARS['products_id'])) {
                                if (tep_has_product_attributes($HTTP_GET_VARS['products_id'])) {
                                  ...
                                  $cart->add_cart($HTTP_GET_VARS['products_id'], $cart->get_quantity($HTTP_GET_VARS['products_id'])+$HTTP_POST_VARS['quantity']);
                                }
                              }
                              tep_redirect(tep_href_link($goto, tep_get_all_get_params($parameters)));
                              break;
...
// selection of product model name in nav bod by gunfas
 if (isset($HTTP_GET_VARS['products_id'])) {
        $model_query = tep_db_query("select p.products_model, pd.products_name from " . TABLE_PRODUCTS . " p, " . TABLE_PRODUCTS_DESCRIPTION . " pd where p.products_id = '" . (int)$HTTP_GET_VARS['products_id'] . "' and pd.products_id = p.products_id and pd.language_id = '" . (int)$languages_id . "'");
        if (tep_db_num_rows($model_query)) {
          $model = tep_db_fetch_array($model_query);
          if(_CFG_SHOW_PRODUCTS_NAME_INNAV == 'true')
...
// set which precautions should be checked
  ...
  require_once('includes/languages/'.$language.'/main.php');
?>
