      ...
      if (tep_not_null($order->info['shipping_method'])) {
        if ($GLOBALS[$module]->tax_class > 0) {
         ...
         $search = array(' regimark', ' tradmrk');
					...
					//NEED THIS BLOCK FOR FLAT PRICE SHIPPING
				$fixed_method=ereg_replace(' \(\)', '' ,
				$order->info['shipping_method']);
				if($fixed_method==' (FREE_SHIPPING_TITLE)'){
					$fixed_method='No Shipping Cost';
				}					

                        $this->output[] = array('title' => str_replace($search, $replace, $fixed_method) . ':',
