...
function quote($method = '')
{
	...
	require_once(DIR_WS_INCLUDES.'library/fedex-common.php');

	$path_to_wsdl = DIR_WS_INCLUDES.'wsdl/RateService_v6.wsdl';	
	
	require_once(DIR_WS_INCLUDES.'librarysoap/nusoap.php');	
	
	require_once(DIR_WS_INCLUDES.'librarysoap/class.wsdlcache.php');

	$cache = new wsdlcache('.', 60);
	
	$wsdl = $cache->get($path_to_wsdl);
	...
	/*$request['RequestedShipment']['Recipient'] = array('Address' => array (
                                               'StreetLines' => array(''), // customer street address
                                               'City' => '', //customer city
                                               'StateOrProvinceCode' => '', //customer state
                                               'PostalCode' => '521', //customer postcode
                                               'CountryCode' => 'US')); //customer county code*/
	...
	//echo '<br>cart_total_weight'. number_format($cart_total_weight,1);
	//echo '<br>total_weight'.$total_weight;
	$request['RequestedShipment']['RequestedPackages'] = array('0' => array('SequenceNumber' => '1',
		...
		//print_r($request['RequestedShipment']['RequestedPackages'])	;																						 
//	try 
//	{
