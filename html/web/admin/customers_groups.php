...
<?php
  ...
  require('includes/application_top.php');
	...
	$cg_tax_rates_array = array();

    $tax_rates_query = tep_db_query("select tax_rates_id, tax_description from " . TABLE_TAX_RATES);

    while($tax_rate = tep_db_fetch_array($tax_rates_query)) {

		$cg_tax_rates_array[] = array('id' => $tax_rate['tax_rates_id'], 'text' => $tax_rate['tax_description']);
  ...
  if (tep_not_null($action)) {
    ...
    switch ($action) {
      ...
      case 'update':
		...
		$cg_tax_exempt_rates = tep_db_prepare_input($HTTP_POST_VARS['customers_group_tax_exempt_rates']);
		...
		if ($customers_group_tax_exempt == 1) {

			tep_db_query("delete from ". TABLE_CUSTOMERS_GROUPS_TAX_RATES_EXEMPT ." where customers_group_id = ". tep_db_input($customers_group_id));

			$cg_tr_values = '';

			foreach ($cg_tax_exempt_rates as $tax_rates_id) {

				$cg_tr_values .= '('.$customers_group_id.','.$tax_rates_id.'),';

			}

			$cg_tr_values = substr($cg_tr_values, 0, -1);

			tep_db_query("insert into ". TABLE_CUSTOMERS_GROUPS_TAX_RATES_EXEMPT ." VALUES ".$cg_tr_values);
      ...
      case 'deleteconfirm':
		...
		tep_db_query("delete from ". TABLE_CUSTOMERS_GROUPS_TAX_RATES_EXEMPT ." where customers_group_id = ". tep_db_input($group_id));
	   ...
	   $group_payment_allowed = substr($group_payment_allowed,0,strlen($group_payment_allowed)-1);
		...
		if ($customers_group_tax_exempt == 1) {
			...
			foreach ($cg_tax_exempt_rates as $tax_rates_id) {
				...
				$cg_tr_values .= '('.$new_cg_id.','.$tax_rates_id.'),';
...
<?php
  ...
  if ($HTTP_GET_VARS['action'] == 'edit') {
	...
	$cg_tr_exempt_query = tep_db_query("select tax_rates_id from ". TABLE_CUSTOMERS_GROUPS_TAX_RATES_EXEMPT ." where customers_group_id = '" . $HTTP_GET_VARS['cID'] . "'");

    while($tax_rate = tep_db_fetch_array($cg_tr_exempt_query)) {

		$cgtr_array[$tax_rate['tax_rates_id']] = true;
   ...
   $customers_groups_tax_rates_exempt_query_5 = tep_db_query("select tr.tax_description, tr.tax_rates_id from " . TABLE_TAX_RATES . " tr inner join " . TABLE_CUSTOMERS_GROUPS_TAX_RATES_EXEMPT . " te on tr.tax_rates_id = te.tax_rates_id where te.customers_group_id = '". $HTTP_GET_VARS['cID'] . "'");

   if ($customers_groups_tax_rates_exempt_5 = tep_db_fetch_array($customers_groups_tax_rates_exempt_query_5)) {

   $taxreadescription = $customers_groups_tax_rates_exempt_5['tax_description'];

   $taxreadescription_id = $customers_groups_tax_rates_exempt_5['tax_rates_id'];
...
//--></script>
      ...
      <tr>
        ...
        <td class="formArea"><table border="0" cellspacing="0" cellpadding="0">
          ...
          <tr>
            ...
            <td class="main" valign="top"><br><?php echo ENTRY_GROUP_TAX_EXEMPT_RATES; ?><br>

<br style="line-height:10px"></td>

            <td class="main" style="line-height: 5px"><?php echo tep_draw_pull_down_menu('customers_group_tax_exempt_rates[]', $cg_tax_rates_array, $taxreadescription_id, '', false, true); ?></td>
      ...
      <tr>
        ...
        <td class="formArea"><table border="0" cellspacing="2" cellpadding="2">
          ...
          <tr>
            ...
            <td class="main" valign="top"><?php echo ENTRY_GROUP_TAX_EXEMPT_RATES; ?></td>

            <td class="main" valign="top"><?php echo tep_draw_pull_down_menu('customers_group_tax_exempt_rates[]', $cg_tax_rates_array, '0', '', false, true); ?></td>
      ...
      <tr>
        ...
        <td><table border="0" width="100%" cellspacing="0" cellpadding="0">
          ...
          <tr><?php echo tep_draw_form('search', 'customers_groups.php', '', 'get'); ?>
            ...
            <td class="smallText2" align="right"><?php echo HEADING_TITLE_SEARCH . ' ' . tep_draw_input_field('search'); ?></td>
