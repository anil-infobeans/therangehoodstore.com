...
while ($customers_group = tep_db_fetch_array($customers_group_query)) // Gets all of the customers groups
  ...
  if (tep_db_num_rows($attributes_query) > 0) {
    ...
    if ($HTTP_POST_VARS['sppcoption'][$customers_group['customers_group_id']]) { // this is checking if the check box is checked
	  ...
	  if (($HTTP_POST_VARS['sppcminorder'][$customers_group['customers_group_id']] <> $attributes['minorder']) && ($attributes['customers_group_id'] == $customers_group['customers_group_id']) ) {

	    tep_db_query("update " . TABLE_PRODUCTS_GROUPS . " set minorder = '" . $HTTP_POST_VARS['sppcminorder'][$customers_group['customers_group_id']] ."' where customers_group_id = '" . $attributes['customers_group_id'] . "' and products_id = '" . $products_id . "'");

        $attributes = tep_db_fetch_array($attributes_query);

      }

      elseif (($HTTP_POST_VARS['sppcminorder'][$customers_group['customers_group_id']] == $attributes['minorder'])) {
...
while ($customers_group = tep_db_fetch_array($customers_group_query)) // Gets all of the customers groups
  ...
  $attributes = tep_db_fetch_array($attributes_query);
            ...
            tep_db_query("insert into " . TABLE_PRODUCTS_GROUPS . " (customers_group_id, customers_group_price, products_id, minorder) values ('" . $attributes['customers_group_id'] . "', '" . tep_db_input($attributes['customers_group_price']) . "', '" . tep_db_input($attributes['minorder']) . "', '" . (int)$dup_products_id . "')");
...
?>
 ...
 &nbsp;</td>
	   ...
	   echo tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' . tep_draw_input_field('sppcprice[' . $customers_group['customers_group_id'] . ']', $sppc_cg_price );
            ...
            <td class="main"><?php echo TEXT_PRODUCTS_MINIMUM; ?>&nbsp;&nbsp;<?php
       ...
       echo tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' . tep_draw_input_field('sppcminorder[' . $customers_group['customers_group_id'] . ']', $attributes['minorder']);
	       ...
	       if (isset($pInfo->sppcminorder[$customers_group['customers_group_id']])) { // when a preview was done and the back button used
