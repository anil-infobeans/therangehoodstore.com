  ...
  if (tep_not_null($action)) {
    ...
    switch ($action) {
      ...
      // ** END GOOGLE CHECKOUT **      
	    ...
	    tep_redirect(tep_href_link(FILENAME_MODULES5, 'set=' . $set . '&module=' . $HTTP_GET_VARS['module']));
      ...
      case 'remove':
        ...
        tep_redirect(tep_href_link(FILENAME_MODULES5, 'set=' . $set . '&module=' . $class));
  ...
  if ($dir = @dir($module_directory)) {
    ...
    while ($file = $dir->read()) {
      ...
      if (!is_dir($module_directory . $file)) {
        ...
        if ((substr($file, strrpos($file, '.')) == $file_extension) || (substr($file, strrpos($file, '.')) == '.php5')){
    ...
    if (tep_class_exists($class)) {
      ...
      if (isset($mInfo) && is_object($mInfo) && ($class == $mInfo->code) ) {
        ...
        if ($module->check() > 0) {
          ...
          echo '              <tr id="defaultSelected" class="dataTableRowSelected" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)" onclick="document.location.href=\'' . tep_href_link(FILENAME_MODULES5, 'set=' . $set . '&module=' . $class . '&action=edit') . '\'">' . "\n";
        ...
        echo '              <tr class="dataTableRow" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)" onclick="document.location.href=\'' . tep_href_link(FILENAME_MODULES5, 'set=' . $set . '&module=' . $class) . '\'">' . "\n";
...
?>
                ...
                <td class="dataTableContent" align="right"><?php if (isset($mInfo) && is_object($mInfo) && ($class == $mInfo->code) ) { echo tep_image(DIR_WS_IMAGES . 'icon_arrow_right.gif'); } else { echo '<a href="' . tep_href_link(FILENAME_MODULES5, 'set=' . $set . '&module=' . $class) . '">' . tep_image(DIR_WS_IMAGES . 'icon_info.gif', IMAGE_ICON_INFO) . '</a>'; } ?>&nbsp;</td>
...
<?php
  ...
  switch ($action) {
    ...
    case 'edit':
      ...
      $contents = array('form' => tep_draw_form('modules', FILENAME_MODULES5, 'set=' . $set . '&module=' . $HTTP_GET_VARS['module'] . '&action=save'));

      $contents[] = array('text' => $keys);

      $contents[] = array('align' => 'center', 'text' => '<br>' . tep_image_submit('button_update.gif', IMAGE_UPDATE) . ' <a href="' . tep_href_link(FILENAME_MODULES5, 'set=' . $set . '&module=' . $HTTP_GET_VARS['module']) . '">' . tep_image_button('button_cancel.gif', IMAGE_CANCEL) . '</a>');
    ...
    default:
      ...
      if ($mInfo->status == '1') {
        ...
        $contents[] = array('align' => 'center', 'text' => '<a href="' . tep_href_link(FILENAME_MODULES5, 'set=' . $set . '&module=' . $mInfo->code . '&action=remove') . '">' . tep_image_button('button_module_remove.gif', IMAGE_MODULE_REMOVE) . '</a> <a href="' . tep_href_link(FILENAME_MODULES5, 'set=' . $set . (isset($HTTP_GET_VARS['module']) ? '&module=' . $HTTP_GET_VARS['module'] : '') . '&action=edit') . '">' . tep_image_button('button_edit.gif', IMAGE_EDIT) . '</a>');
        ...
        $contents[] = array('align' => 'center', 'text' => '<a href="' . tep_href_link(FILENAME_MODULES5, 'set=' . $set . '&module=' . $mInfo->code . '&action=install') . '">' . tep_image_button('button_module_install.gif', IMAGE_MODULE_INSTALL) . '</a>');
