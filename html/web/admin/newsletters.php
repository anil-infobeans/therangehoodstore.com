  ...
  if ($HTTP_GET_VARS['action']) {
    ...
    switch ($HTTP_GET_VARS['action']) {
      ...
      case 'update':
        ...
        $image->set_destination(DIR_FS_CATALOG_IMAGES);
        ...
        if ($image->parse() && $image->save()) {
          ...
          $image_name = (isset($HTTP_POST_VARS['previous_image']) ? $HTTP_POST_VARS['previous_image'] : '');
		      ...
		      if ($image = new upload('image', DIR_FS_CATALOG_IMAGES)) {

            tep_db_query("update " . TABLE_NEWSLETTERS . " set image = '" .  $image_name . "' where newsletters_id = '" . tep_db_input($newsletter_id) . "'"); }
...
<title><?php echo TITLE . ' - ' . HEADING_TITLE . ' - Mode: ' .  HTML_AREA_WYSIWYG_BASIC_NEWSLETTER . ' - HTML Status: ' . HTML_AREA_WYSIWYG_DISABLE_NEWSLETTER;?></title>
...
//MaxiDVD Added WYSIWYG HTML Area Box + Admin Function v1.6.5 Newsletter <head>
...
<?php
  ...
  if ($HTTP_GET_VARS['action'] == 'new') {
    ...
    if ($HTTP_GET_VARS['nID']) {
      ...
      $newsletter_query = tep_db_query("select title, image, content, module from " . TABLE_NEWSLETTERS . " where newsletters_id = '" . tep_db_input($nID) . "'");
...
?>
      ...
      <tr><?php echo tep_draw_form('newsletter', FILENAME_NEWSLETTERS, 'page=' . $HTTP_GET_VARS['page'] . '&action=' . $form_action, 'post', 'enctype="multipart/form-data"');if ($form_action == 'update') echo tep_draw_hidden_field('newsletter_id', $nID); ?>
        ...
        <td><table border="0" cellspacing="0" cellpadding="2">
          ...
          <tr>
            ...
            <td class="main">Imagen del Boletín</td>

            <td class="main"><?php echo tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' . tep_draw_file_field('image') . '<br>' . tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' . $nInfo->image . tep_draw_hidden_field('previous_image', $nInfo->image); ?></td>
          ...
          <tr>
			...
			<?php tep_draw_fckeditor('content','700','500',$nInfo->content); ?>	
  ...
  <?php if (HTML_AREA_WYSIWYG_DISABLE_NEWSLETTER == 'Disable') { '<br>' . include ("comment_newsletter_bar.php");} else { ?>
   ...
   ?>
      ...
      <tr>
        ...
        <td><?php echo tep_draw_separator('pixel_trans.gif', '1', '10'); ?><?php echo  tep_draw_hidden_field('image', stripslashes($image_name)); ?></td>
